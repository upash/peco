...
class Peco {
  ...
  getPermalink(filepath, { date, type } = {}) {
    const title = encodeURI(filepath.replace(/^_posts\//, '').replace(/\.md$/, ''))
  ...
  async buildFiles({ filepath } = {}) {
    ...
    await Promise.all(
      Array.from(this.files.entries()).map(async entry => {
        ...
        if (data.attributes.type === 'index') {
          ...
          await Promise.all(
            new Array(totalPages).fill(null).map(async (_, index) => {
              ...
              const outFile = this.resolvePecoDir(
                'data',
                `${route === '/' ? '/index' : route}.json`
              )
              this.addRouteFromPath(
                outFile.replace(this.resolvePecoDir(), 'dot-peco'),
                route
              )
