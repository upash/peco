...
class Peco {
  ...
  async handleConfig(config = {}) {
    ...
    if (/^[./]|(^[a-zA-Z]:)/.test(theme)) {
      ...
      theme = this.resolveLocalModule(`peco-theme-${theme}`)
    }
    ...
    if (themeConfig.engine) {
      this.Engine = this.requireLocalModule(`peco-engine-${themeConfig.engine}`)
    } else {
    ...
    return {
      ...
      markdown: config.markdown || this.config.markdown || {}.baseDir,
      ga_id: config.ga_id || this.config.ga_id
    }
  }

  requireLocalModule(name) {
  ...
  async getFileData(filepath, stats) {
    ...
    if (markdown.anchor !== false) {
      ...
      if (typeof markdown.slugify === 'string') {
        slugify = this.requireLocalModule(markdown.slugify)
    ...
    if (markdown.plugins) {
      if (typeof markdown.plugins === 'function') {
        ...
        for (const plugin of markdown.plugins) {
          md.use(
            this.requireLocalModule(plugin.name),
            plugin.options
          )
        }
      }
