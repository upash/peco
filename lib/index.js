...
class Peco {
  ...
  getPermalink(filepath, { date, type } = {}) {
    ...
    if (type === 'post') {
      ...
      if (this.config.localeNames) {
        for (const name of this.config.localeNames) {
          const RE = new RegExp(`^${name}[/$]`)
          if (RE.test(slug)) {
            slug = slug.replace(RE, '')
            // Do not add lang prefix for default locale
            // eslint-disable-next-line max-depth
            if (name !== this.config.defaultLocale) {
              langPrefix = `${name}/`
            }
            break
          }
      ...
      const link = langPrefix + this.config.permalink
        .replace(/:year/, year)
        .replace(/:month/, month)
  ...
  addRouteFromPath(filepath, route) {
    ...
    if (file.data.attributes.type !== 'index') {
      this.routes.set(file.data.permalink, {
        path: `dot-peco/data${file.data.permalink}.peson`,
        type: 'peson'
      })
    }
  }
  ...
  async onChangeFile(filepath) {
    const stats = await fs.stat(
      this.resolveSourceDir(filepath)
    )
    this.files.set(filepath, {
      stats,
      data: await this.getFileData(filepath, stats)
    ...
    await this.buildFiles({ filepath })
    // For now no need to update route for changed files
    // Since we get route from filepath
    // But in the future you may use `slug` in front-matter to set the route
  }
  ...
  async buildFiles({ filepath } = {}) {
    ...
    await Promise.all(
      files.map(async ([filepath, file]) => {
        ...
        const outFile = this.resolvePecoDir('data', `${data.permalink}.peson`)
