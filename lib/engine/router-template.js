    ...
    .map(([path, item]) => {
      return `{
        ...
        component: async () => {
          ${
  item.type === 'peson' ?
    `return import(/* webpackChunkName: "${pathToComponentName(path)}" */'${item.path}?name=${pathToComponentName(path)}')` :
    item.type === 'component' ? /* handle component */ `
    ...
    ` : `return import(/* webpackChunkName: "${pathToComponentName(path)}" */ '${item.path}')`}
