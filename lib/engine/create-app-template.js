...
const { slash } = require('./app/utils')

const pathToId = file => {
  return path.basename(file).replace(/[^a-zA-Z0-9_]/g, '_')
}

module.exports = _enhanceAppFiles => {
  const enhanceAppFiles = _enhanceAppFiles.map(file => slash(file))
...
const r = require.context('@theme-src', false, /index\\.js$/)
let enhanceApp
r.keys().forEach(fp => {
  if (fp === './index.js') {
