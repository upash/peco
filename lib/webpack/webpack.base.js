...
module.exports = (ctx, type) => {
  ...
  config.resolve.alias
    ...
    .set(
      '@noop-query$',
      path.join(__dirname, 'peco-query-loader', 'noop.pecoquery')
    )
  ...
  config.module
    .rule('pecoquery')
    .test(/\.pecoquery$/)
    .use('peco-query-loader')
      .loader(require.resolve('./peco-query-loader'))
      .options({
        api: ctx
      })
  ...
  config.module
    ...
    .options({
      ...
      plugins: [
        ...
        require.resolve('../babel/query-plugin')
      ]
    })
  ...
  config.plugin('constants').use(require('webpack').DefinePlugin, [
    {
      ...
      __PUBLIC_PATH__: JSON.stringify(ctx.config.root)
    }
  ])
