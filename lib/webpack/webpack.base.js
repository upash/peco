...
module.exports = (ctx, type) => {
  ...
  if (ctx.mode === 'production' && ctx.options.minimize !== false) {
    config.merge({
      optimization: {
        minimize: true,
        minimizer: [
  ...
  config.resolve.alias
    .set('@theme', ctx.config.theme)
    .set('dot-peco', ctx.resolvePecoDir())
    .set('@site-meta', ctx.resolvePecoDir('data/__siteMeta.json'))
  ...
  config.plugin('constants')
    .use(require('webpack').DefinePlugin, [{
      ...
      'process.env.GA_ID': ctx.config.ga_id ? JSON.stringify(ctx.config.ga_id) : false
    }])
